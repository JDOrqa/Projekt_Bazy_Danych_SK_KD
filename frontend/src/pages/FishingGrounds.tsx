...
export function FishingGrounds() {
  ...
  const [grounds, setGrounds] = useState<FishingGround[]>([]);
  ...
  useEffect(() => {
    const fetchGrounds = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/api/lowiska`, {
          headers: token ? { Authorization: `Bearer ${token}` } : {}
        });
        console.log('Fetched fishing grounds:', response.data);
        setGrounds(response.data);
        setError(null);
      } catch (err: any) {
        console.error('Error fetching fishing grounds:', err);
        setError('Błąd pobierania łowisk');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredGrounds = grounds.filter(ground =>
    ground.nazwa.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ground.typ.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && (
        ...
        >
          <Card className="p-12 text-center">
            <p className="text-gray-600">Ładowanie łowisk...</p>
          </Card>
        </motion.div>
      )}
      ...
      {/* Grounds List */}
      {!loading && !error && filteredGrounds.length === 0 ? (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
            ...
            >
              ...
              >
                ...
                <div className="space-y-2 mb-4 flex-1">
                  {ground.powierzchnia_ha && (
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600">Powierzchnia:</span>
                      <span className="font-medium text-gray-900">{ground.powierzchnia_ha} ha</span>
                    </div>
                  )}
                  {ground.glebokosc_max && (
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600">Maks. głębokość:</span>
                      <span className="font-medium text-gray-900">{ground.glebokosc_max} m</span>
                    </div>
                  )}
                </div>
